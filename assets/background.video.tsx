...
const ambientWashStyle = (seconds: number): CSSProperties => ({
  background: [
    // 主题色光晕（左上：主强调色）
    `radial-gradient(820px 560px at ${16 + Math.sin(seconds * 0.22) * 5}% ${6 + Math.cos(seconds * 0.18) * 4}%, ${colors.glowA}, transparent 66%)`,
    // 暖色光晕（右下）
    `radial-gradient(780px 540px at ${84 + Math.cos(seconds * 0.2) * 5}% ${20 + Math.sin(seconds * 0.19) * 4}%, ${colors.glowB}, transparent 64%)`,
    // 冷色光晕（右下辅助，与暖色错开）
    `radial-gradient(640px 460px at ${70 + Math.sin(seconds * 0.16) * 6}% ${86 + Math.cos(seconds * 0.21) * 4}%, ${colors.glowC}, transparent 62%)`,
    `linear-gradient(180deg, ${colors.canvasTop} 0%, ${colors.canvasMid} 46%, ${colors.canvasBottom} 100%)`,
  ].join(", "),
});
